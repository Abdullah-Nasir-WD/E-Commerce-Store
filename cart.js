...
convert.map((item , index) => {
    ...
    <div class="alone-card">
        ...
        <h2 class="bg-red">${item.warrantyInformation}</h2></br>
        <h3 class="bg-green" id="price${index}">Price : $${item.price}</h3>
        <p>${item.description.slice(0, 30)}<b>......</b></p>

        <h2 class="d-flex gap-3 justify-content-center" > Quantity : 
        ...
        <button class="order-btn" onclick="deleteCard(${index})">Delete</button>

        <button class="order-btn" onclick="buyNow(${index})">Buy Now</button>
